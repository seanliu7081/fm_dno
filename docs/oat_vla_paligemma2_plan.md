...
2. **Enriched-past AR policy** ([train_past2next.yaml](../oat/config/train_past2next.yaml)): conditions on 2 obs frames + 7 raw past actions (acc/jerk/raw MLP features); small AR head predicts the 8 tokens; executes 8 of 16 steps; rolling `_past_buffer`. Baseline SR 0.824.
...
- Enriched-past scheme ported verbatim from [past2next.py](../oat/policy/past2next.py) (L253-272): acc = a₋₁−a₋₂, jerk = a₋₁−2a₋₂+a₋₃ on normalized past actions; MLPs `Linear(in→D)→GELU→Linear(D→D)`.
