...
```
  ...
  augmented actions  ──►  LinearNormalizer (limits → [-1, +1])
                       ...
                       ╔════════════════════════════════════════════════╗
                       ║  STAGE 2 — POLICY  (Past2NextPolicy) ║
                       ╚════════════════════════════════════════════════╝
...
│   ├── train_past2next.yaml        [Stage 2 hydra entry]
│   └── task/
...
| 2     | [oat/config/train_past2next.yaml](oat/config/train_past2next.yaml) (override `policy.action_tokenizer.checkpoint=<stage1.ckpt>`) | `oat.workspace.train_policy.TrainPolicyWorkspace` | `*.ckpt` of full policy (incl. tokenizer weights) |
...
## 5. Stage 2 — `Past2NextPolicy` architecture

File: [oat/policy/past2next.py](oat/policy/past2next.py).
