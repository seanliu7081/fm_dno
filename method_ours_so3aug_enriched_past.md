...
│   ├── train_past2next.yaml     [Stage 2 hydra entry point]
│   └── task/
...
| 2 | `config/train_past2next.yaml` (override `policy.action_tokenizer.checkpoint=<stage1.ckpt>`) | `oat.workspace.train_policy.TrainPolicyWorkspace` | `*.ckpt` containing the full policy (tokenizer + AR backbone + projections) |
...
### 3.2 Policy side — `Past2NextPolicy`

File: [oat/policy/past2next.py](oat/oat/policy/past2next.py).
