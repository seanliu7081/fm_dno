...
usage() {
  cat <<'HELP'
Usage: bash train_DiTL_nut_washer_v3.sh --gpu ID[,ID...] [options]
...
Examples:
  bash train_DiTL_nut_washer_v3.sh --gpu 0
  bash train_DiTL_nut_washer_v3.sh --gpu 0,1 --batch-size 32
  bash train_DiTL_nut_washer_v3.sh --gpu 1 --dry-run

Dataset: /workspace/ysk/zarr/nut_washer_v3_N77.zarr (77 demonstrations).
Model: DiT width 512, 8 blocks, 8 heads; observation token width 256.
...
RUN=${RUN_DIR:-$PWD/output/train_DiTL_nut_washer_v3_$(date -u +%Y%m%d_%H%M%S)}
RUN=$(realpath -m -- "$RUN")
OVERRIDES=(
  --config-name=train_DiTL_nut_washer_v3
