...
2. **Stage 2 — Autoregressive policy** (`train_past2next.yaml`):
...
(`train_past2next.yaml:43`) — you must train Stage 1 first and
...
## 3. Stage 2 — The policy (`train_past2next.yaml`)

Class: `Past2NextPolicy` (`oat/policy/past2next.py`).
...
> (obs + projected raw past) ⊂ `Past2NextPolicy` (obs + raw past + acc/jerk).
...
### 3.6 Config cheat-sheet (`train_past2next.yaml`)
...
python oat/workspace/train_policy.py --config-name train_past2next \
